		...
		for _, fieldCache := range structCache.Children {
			...
			if val.IsNil() {
				...
				if cache.IsRedisError(err) {
					logger.Error("%v cache err:%v", cacheKey, err.Error())
				}
				continue
			}
			fieldInterface := val.Interface()
			...
			if !dirtyMark.HasCached() {
				...
				dirtyMark.RangeDirtyMap(func(dirtyKey interface{}, isAddOrUpdate bool) {
					if isAddOrUpdate {
						...
						// delete
						delMap = append(delMap, Itoa(dirtyKey))
					}
				})
				if len(setMap) > 0 {
					// 批量更新
...
// 从缓存中恢复数据
func LoadFromCache(kvCache KvCache, obj interface{}, cacheKey string) (bool, error) {
	structCache := GetSaveableStruct(reflect.TypeOf(obj))
	if structCache == nil {
		return false, nil
	}
	...
	if structCache.IsSingleField() {
		...
		for _, fieldCache := range structCache.Children {
			...
			fieldInterface := val.Interface()
			hasCache, err := LoadFromCache(kvCache, fieldInterface, cacheKey+"."+fieldCache.Name)
			if !hasCache {
				continue
			}
			if err != nil {
