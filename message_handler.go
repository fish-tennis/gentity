...
func (this *MessageHandlerRegister) scanMethods(obj any, packetHandlerRegister gnet.PacketHandlerRegister,
	...
	for i := 0; i < typ.NumMethod(); i++ {
		...
		GetLogger().Debug("ScanPacketHandler %v.%v %v client:%v", componentStructName, method.Name, messageId, isClientMessage)
	}
}
			...
			if component != nil {
				if handlerInfo.Handler != nil {
					...
					if !handlerInfo.Method.Func.IsValid() {
						GetLogger().Error("InvokeErr method invalid cmd:%v", packet.Command())
						return false
					}
					// 反射调用函数
